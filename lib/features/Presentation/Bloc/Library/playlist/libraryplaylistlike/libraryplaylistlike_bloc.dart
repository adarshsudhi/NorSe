...
class LibraryplaylistlikeBloc extends Bloc<LibraryplaylistlikeEvent, LibraryplaylistlikeState> {
  LibraryplaylistlikeBloc() : super(const _Initial()) {
    on<_Addedtolibrary>((event, emit) async{
      Either<Failures, String> res = await di.di<Addtolibraryusecase>().call(event.id,event.song);
      res.fold((l) {
