...
class FavsongsBloc extends Bloc<FavsongsEvent, FavsongsState> {
  FavsongsBloc() : super(const _Initial()) {
    on<_Checkforfavsong>((event, emit) async{
      Either<Failures,bool> res = await di.di<Getfavsongstatususecase>().call(event.songid);
      res.fold((l) {
      ...
      on<_Addtofav>((event, emit)async {
       Either<Failures,String> res = await di.di<AddtofaveUsecase>().call(event.song,event.isadded); 
       res.fold((l) {
       log('Fav song failed');
       ...
       Either<Failures,bool> res = await di.di<removefromfavusecase>().call(event.id);
       res.fold((l) {
        log('not such item');
       }, (r) {
        if (r == true) {
