...
class PlaylistsongsBloc extends Bloc<PlaylistsongsEvent, PlaylistsongsState> {
  ...
  PlaylistsongsBloc(this.songsusecase) : super(const _Initial()) {
    on<_Getallsongs>((event, emit) async{
       Either<Failures, List<Map<String, dynamic>>> res = await songsusecase.call(event.playlistid);
       res.fold((l) {
        log('playlist song failed');
        emit(const PlaylistsongsState.songs([]));
       }, (r) => 
       emit(PlaylistsongsState.songs(r)));
    });
    on<_Deletesong>((event, emit) async{
      Either<Failures,bool> res =await di.di<Removesongfromplaylistusecase>().call(event.playlistid,event.songid);
      res.fold((l) {
        log('Remove failed');
      }, (r) {
        log('Removed');
