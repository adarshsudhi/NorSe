...
class LibraryalbumlikeBloc extends Bloc<LibraryalbumlikeEvent, LibraryalbumlikeState> {
  LibraryalbumlikeBloc() : super(const _Initial()) {
     on<_Addedtolibrary>((event, emit) async{
       Either<Failures,String>  res = await di.di<Addtolibraryusecase>().call(event.type,event.song);
       res.fold((l) {
        log('Failed');
       }, (r){
        if (r == 'Added album') {
    ...
    on<_Check>((event, emit) async{
      bool ispresent = await di.di<Checkforalbumpresentusecase>().call(event.id);
