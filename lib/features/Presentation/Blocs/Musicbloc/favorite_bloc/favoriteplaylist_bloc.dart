...
import 'package:nebula/features/Domain/UseCases/Sql_UseCase/getallfav_Usecase.dart';
...
class FavoriteplaylistBloc extends Bloc<FavoriteplaylistEvent, FavoriteplaylistState> {
  FavoriteplaylistBloc() : super(const _Initial()) {
    on<_Getallsongs>((event, emit) async{
      Either<Failures,List<Map<String,dynamic>>> response = await di.di<GetallfaveUsecase>().call();
      response.fold((l) {
    ...
    on<_Removesongs>((event, emit) async{
      log('working');
       Either<Failures,bool> response = await di.di<removefromfavusecase>().call(event.id);
    ...
    on<_Updatelist>((event, emit) async{
      Either<Failures, List<Map<String, dynamic>>>  res =await di.di<updateplaylistusecase>().call('fav',event.newlist);
      res.fold((l) {
        log('update failed');
      }, (r) {
        log('updated');
