...
import 'package:nebula/features/Domain/UseCases/Sql_UseCase/addtorecents_UseCase.dart';
...
class LocaldataBloc extends Bloc<LocaldataEvent, LocaldataState> {
  ...
  LocaldataBloc(
    this.getDownloads,
    this.addtorecent, this.removefromrecentsUsecase,
  ) : super( const _Initial(false)) {


    on<_GetDownloads>((event,emit) async{
       state.mapOrNull(querys: (value) => emit(value.copyWith(isloading: true)),);
       Either<Failures,List<Map<String,dynamic>>> Res = await getDownloads.call();
       Res.fold((fail) {
    ...
    on<_Clearalldownloads>((event, emit) async{
        Either<Failures,bool> res = await di.di<Clearalldownloadusecase>().call();
        res.fold((l) =>{
          log('downloads remove failed')
        }, (r) {
    ...
    on<_RemovefromDownloads>((event, emit) async{
      Either<Failures, bool> res = await di.di<RemovefromdownloadsUseCase>().call(event.id);
     res.fold((l) { 
    ...
    on<_Removefromrecents>((event, emit) async{
          Either<Failures,bool> res = await removefromrecentsUsecase.call(event.id);
       res.fold((l) {
        log('no such item');
       } , (r) {
        if (r) {
