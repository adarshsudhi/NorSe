  ...
  PlaylistBloc() : super(const _Initial()) {
    on<_Createplaylist>((event, emit) async {
      Either<Failures, bool> res =
          await di.di<Createplaylistusecase>().call(event.playlistname);
      res.fold((l) {
        log('playlist Failed');
      }, (r) {
    ...
    on<_Removeplaylist>((event, emit) async {
      Either<Failures, bool> res = await di
          .di<Removeplaylistusecase>()
          .call(event.playlistid, event.playlistname);
      res.fold((l) {
        log('Remove failed');
      }, (r) {
    ...
    on<_Addtoplaylist>((event, emit) async {
      Either<Failures, bool> res =
          await di.di<Addtoplaylistusecase>().call(event.tbname, event.song);
      res.fold((l) {
        log('adding failed');
      }, (r) {
        if (r) {
